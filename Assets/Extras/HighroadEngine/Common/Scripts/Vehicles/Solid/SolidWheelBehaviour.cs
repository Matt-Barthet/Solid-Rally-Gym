...
{		
	...
	public class SolidWheelBehaviour : MonoBehaviour 
	{
		...
		private float startingPrev, startingCurrent, startingSpring, startingWheel;
		...
		protected virtual void Start() 
		{
			...
			startingPrev = _previousLength;
			startingCurrent = _currentLength;
			startingSpring = _springVelocity;
			startingWheel = _wheelDistance;
		}

        public void Reset()
        {
			_previousLength = startingPrev;
			_currentLength = startingCurrent;
			_springVelocity = startingSpring;
			_wheelDistance = startingWheel;
        }

		public void SetProperties(float previous, float current, float spring, float wheel)
        {
			_previousLength = previous;
			_currentLength = current;
			_springVelocity = spring;
			_wheelDistance = wheel;
        }

		public float[] GetProperties()
        {
			return new float[4]{ _previousLength, _currentLength, _springVelocity, _wheelDistance};
        }
		...
		protected virtual void UpdateWheelHeight()
		{
			if (Physics.Raycast(transform.position, -transform.up, out _updateHit, _wheelDistance))
			{
				...
				{
					_wheelTargetPosition.y = Mathf.Lerp(WheelPosition.position.y, _wheelTargetPosition.y, Time.fixedDeltaTime * 4);
				}
			}
			else
			{
				...
				_wheelTargetPosition.y = Mathf.Lerp(WheelPosition.position.y, _wheelTargetPosition.y, Time.fixedDeltaTime * 2);
			}
		...
		void UpdateWheelRolling()
		{
			...
			{
				rotationAmount = Vector3.right * WheelRotationSpeed * Time.fixedDeltaTime * VehicleController.NormalizedSpeed * Mathf.Sign(VehicleController.CurrentGasPedalAmount);
			...
			else if (VehicleController.IsGrounded)
			{
				...
				rotationAmount = Vector3.right * WheelRotationSpeed * Time.fixedDeltaTime * VehicleController.NormalizedSpeed;
