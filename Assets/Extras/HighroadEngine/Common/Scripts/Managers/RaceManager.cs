...
namespace MoreMountains.HighroadEngine {
    ...
    public class RaceManager : MonoBehaviour {
        [Header("Game Type")]
        [Information("Set to 'tiny', 'solid' or 'apex'.", InformationAttribute.InformationType.Info, false)]
        public string game;
        [Range(0f, 200f)]
        public float maxTimeOnLevel = 3f;
        ...
        public Dictionary<int, SolidController> Players { get; protected set; }

        /// The current race elapsed time. Used in the ranking
        public static float _currentGameTime;
        ...
        public static int TargetFrameRate = 200;
        ...
        public UnityAction<SolidController> OnDisableControlForPlayer;
        ...
        public delegate List<SolidController> OnUpdatePlayersListDelegate();
        public OnUpdatePlayersListDelegate OnUpdatePlayersList;

        // Exposed variables of car transforms
        ...
        public virtual void Start() {
            ...
            Players = new Dictionary<int, SolidController>();
        ...
        protected virtual void InitTestMode() {
            ...
            if (TestHumanPlayers != null) {
                foreach (var player in TestHumanPlayers) {
                    if (player.GetComponent<SolidController>() != null) {
                        LocalLobbyManager.Instance.AddPlayer(new LocalLobbyPlayer {
                            Position = currentPosition,
                            Name = player.GetComponent<VehicleInformation>().LobbyName,
                            Object = player,
            ...
            if (TestBotPlayers != null) {
                foreach (var player in TestBotPlayers) {
                    if (player.GetComponent<SolidController>() != null && player.GetComponent<VehicleAI>() != null) {
                        LocalLobbyManager.Instance.AddPlayer(new LocalLobbyPlayer {
                            Position = currentPosition,
                            Name = player.GetComponent<VehicleInformation>().LobbyName + " " + (currentPosition),
                            VehicleName = player.name,
                            VehicleSelectedIndex = -1,
                            IsBot = true
                        });
        ...
        public bool instantiatePlayers = false;
        ...
        protected virtual void InitPlayers() {
            ...
            if (playersAtStart.Count > StartPositions.Length) {
                ...
                foreach (ILobbyPlayerInfo item in playersAtStart) {
                    ...
                    try
                    {
                        newPlayer = TestBotPlayers[_currentStartPosition];
                    }
                    catch (Exception)
                    {
                        newPlayer = TestHumanPlayers[0];
                    }
                    ...
                    var car = newPlayer.GetComponent<SolidController>();
        ...
        public virtual void Update() {
            ...
            if (_isPlaying) {
                if (Input.GetKeyDown(KeyCode.R)) {
                    GameObject vehicle = Players[0].gameObject;
                    if (game == "tiny") {
                        vehicle.GetComponent<CarController>().Respawn();
                    } else if (game == "solid") {
                        vehicle.GetComponent<SolidController>().Respawn();
                    }
                }
                ...
                if (EndGamePanel == null) {
                    ...
                    if (playersRank.Count > 0) {
                        // update score screen
                        if (PositionText != null && LapText != null) {
                            ...
                            foreach (var p in playersRank) {
                                ...
                                if (ClosedLoopTrack) {
                                    if (p.tag == "Player") {
                                        lap += string.Format("lap {0}/{1}\r\n",
                                            p.CurrentLap >= Laps ? Laps : p.CurrentLap + 1,
                                            Laps
                                        );
                                    }
                                }
                                p._standing = position;
                        ...
                        if (FirstFinisherEndsRace) {
                            ...
                            foreach (SolidController player in playersRank) {
        ...
        protected virtual List<SolidController> UpdatePlayersList() {
            var cars = new List<SolidController>();
        ...
        protected virtual void ShowFinalRanking(List<SolidController> playersRank) {
        ...
        /// </summary>
        public virtual void DisableControlForPlayer(SolidController controller) {
            if (_isNetworkMode) {
                Debug.LogError("This function should not be called in network mode");
                return;
            }
